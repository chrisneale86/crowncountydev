...
RegisterCommand("PlayAnimation", function(source, args, rawCommand)
	...
	local ikFlag1 = args[4] and (1 << tonumber(args[4])) or 0
	local Filter = args[5] or ""

	local combinedIkFlag = ikFlag1
	TaskPlayAnim(ped, dict, name, 1.0, -1.0, duration or -1, flag, 0.0, false, combinedIkFlag, false, Filter, false)
...
-- 	local scenarioIndex = tonumber(args[1] or 1)
-- 	local scenarios

-- 	if GetEntityModel(PlayerPedId()) == GetHashKey("mp_female") then
-- 		scenarios = FemaleCompatibleScenarios
-- 	elseif GetEntityModel(PlayerPedId()) == GetHashKey("mp_male") then
-- 		scenarios = MaleCompatibleScenarios
-- 	else
-- 		scenarios = Scenarios
-- 	end
-- 	if not scenarioIndex then
-- 		print("Please provide a scenario index.")
-- 	else
-- 		if scenarioIndex and scenarios[scenarioIndex] then
...
-- 			local offsetX = 0.5 * math.sin(radHeading)
-- 			local offsetY = -0.5 * math.cos(radHeading)

-- 			TaskStartScenarioAtPosition(PlayerPedId(), scenario, PlayerCoords.x + offsetX, PlayerCoords.y + offsetY, PlayerCoords.z - 0.5, heading, -1, false, true)
-- 			ResetThreadIfStopped(scenario)
-- 		else
-- 			print("Invalid scenario index. Please provide a valid number.")
-- 		end
-- 	end
-- end)


-- RegisterCommand("startscenario", function(source, args)
-- 	local scenario = args[1]
-- 	if not scenario then
-- 		print("Please provide a scenario name.")
-- 		return
-- 	end
...
-- RegisterCommand("sitchair", function(source, args)
-- 	local scenarioIndex = tonumber(args[1] or 1)
-- 	local scenarios
...
-- 			TaskStartScenarioAtPosition(PlayerPedId(), scenario, PlayerCoords.x + offsetX, PlayerCoords.y + offsetY, PlayerCoords.z - 0.5, heading, 0, 1, 0, 0, 0, 0, 0)
-- 			ResetThreadIfStopped(scenario)
-- 		else
-- 			print("Invalid scenario index. Please provide a valid number.")
-- 		end
