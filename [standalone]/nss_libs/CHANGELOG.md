...
- HOTFIX: Fix wrong usage of native `PerformHttpRequest` added with version 0.35.2.
- Validate timestamp format for `NssLibsDiscordEmbed:setTimestamp` to ensure newer FiveM versions are supported.
...
- Add more error information to `NssLibsDiscord:send` for better debugging.
...
- Add `cleanUpItemFromCraftedItemsTable` to inventory api (needed for nss_quests 1.6.4).
