...
- Add `restrict_view_by_callback` to poster config to allow custom view restrictions via callback function, e.g.
  for subscriptions.
- Add `restrict_create_by_callback` to poster config to allow custom creation restrictions via callback function, e.g.
  for subscriptions.
- Add `nss_pinboard:reInitialize` client event to re-initialize pinboard data without reloading the resource, e.g. if
  subscription status changed.

### What must be considered during update?

- Compare your `config.demo.lua` with your `config.lua` to consider new options (see `New since 1.4.0` marks).
...
- Transfer your existing webhook configuration from `config.lua` to new `secure.lua` file (see `New since 1.3.5` marks).
...
- Compare your `config.demo.lua` with your `config.lua` to consider new options (see `New since 1.3.2` marks).

## 1.3.2 (_2024-11-10_)

- Add optional `Config.UseZAxisForOpenPromptRadius` to enable z axis check for open prompt. See `config.demo.lua` for 
  more information.
