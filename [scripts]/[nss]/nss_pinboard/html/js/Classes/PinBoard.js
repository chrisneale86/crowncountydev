...
class PinBoard {
    ...
    _canUseCreateButton() {
        ...
        // Both restrictions results are true by default in case there are no restrictions at all (can be overwritten later).
        let job_restrictions_result = true;
        let callback_restriction_result = true;

        /**
         * @type {PinBoardBoardJobRestrictions|null}
         */
        const job_restrictions = this._current_pinboard_cfg.restrict_create_by_jobs || null;
        const has_job_restrictions = job_restrictions !== null;

        /**
         * @type {boolean|null}
         */
        let can_create_by_callback = null;

        if (typeof this._current_pinboard_cfg.can_create_by_callback_result == 'boolean') {
            can_create_by_callback = this._current_pinboard_cfg.can_create_by_callback_result;
        }

        const has_callback_restriction = can_create_by_callback !== null;

        // If job restrictions exist then set callback result to false (maybe will be overwritten later back to true)
        // because default can not be true if there are job restrictions
        if (has_job_restrictions) {
            callback_restriction_result = false;
        }

        // If callback restriction exist then set job restrictions result to false (maybe will be overwritten later back to true)
        // because default can not be true if there are callback restrictions
        if (has_callback_restriction) {
            job_restrictions_result = false;
        }

        // Evaluate job restrictions if exist and override result
        if (has_job_restrictions) {
            job_restrictions_result = this._canUseCreateButtonByJobRestrictions(job_restrictions);
        }

        // Evaluate callback restriction if exist and override result
        if (has_callback_restriction) {
            callback_restriction_result = can_create_by_callback === true;
        }

        return job_restrictions_result === true || callback_restriction_result === true;
    }

    /**
     * @param {PinBoardBoardJobRestrictions} restrictions
     * @returns {boolean}
     * @private
     */
    _canUseCreateButtonByJobRestrictions(restrictions) {
        ...
        const can_create = current_job_grade >= required_job_grade;
