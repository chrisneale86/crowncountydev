...
local render = {}
local ready
local allowfarming = true 
local disableinv = false 
local updatingnow = false 
RegisterNetEvent("syn_farming:cbuck")
AddEventHandler("syn_farming:cbuck", function()
    ExecuteCommand(Config.removebucketcommand)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if charid ~= nil then 
            ...
            TriggerServerEvent("syn_farming:requestcoords")
        end
    end
end)

RegisterNetEvent("syn_farming:reccoords")
AddEventHandler("syn_farming:reccoords", function(plantcoords)
    local plantstorender = {}

    local coords = GetEntityCoords(PlayerPedId())
    for k,v in pairs(plantcoords) do 
        local dist = GetDistanceBetweenCoords(coords,v.x,v.y,v.z, true)
        if 50 > dist then 
            table.insert(plantstorender, k)
        end
    end
    TriggerServerEvent("syn_farming:requestplants", plantstorender)
end)
...
RegisterNetEvent("syn_farming:reccharid")
AddEventHandler("syn_farming:reccharid", function(x)
    charid = x 
end)
...
AddEventHandler("vorp:SelectedCharacter", function(x)
    charid = x 
    TriggerServerEvent("syn_farming:requestcoords")
    TriggerServerEvent("syn_farming:requestblips")

end)

RegisterNetEvent("syn_farming:recblips")
AddEventHandler("syn_farming:recblips", function(plants)
    if charid ~= nil then 
        allfarms[charid] = plants
    end
    for x,y in pairs(plants) do 
...
Citizen.CreateThread(function()
    ...
    if Config.debug then 
        TriggerServerEvent("syn_farming:requestcharid")
        TriggerServerEvent("syn_farming:requestcoords")
        TriggerServerEvent("syn_farming:requestblips")
...
Citizen.CreateThread(function()
    ...
    local str = language.update
	update = PromptRegisterBegin()
	PromptSetControlAction(update, Config.keys["up"])
	str = CreateVarString(10, 'LITERAL_STRING', str)
	PromptSetText(update, str)
	PromptSetEnabled(update, 1)
    PromptSetVisible(update, 1)
	PromptSetStandardMode(update,1)
	PromptSetGroup(update, prompts)
	Citizen.InvokeNative(0xC5F428EE08FA7F2C,update,true)
	PromptRegisterEnd(update)
...
RegisterNetEvent('syn_farming:recinfo')
AddEventHandler('syn_farming:recinfo', function(x,y)
    allfarms = x 
    ready = y 
end)

RegisterNetEvent('syn_farming:recplant')
AddEventHandler('syn_farming:recplant', function(k,x,g,info,id)
    if info == nil then 
        local k,x,g = findplant(id)
        TriggerEvent("syn_farming:removeplant",k,x,g)
        return
    end
    if  allfarms[k] == nil then 
        allfarms[k] = {}
    end
    if allfarms[k][x] == nil then 
        allfarms[k][x] = {}
    end
    allfarms[k][x][g] = info
end)

RegisterNetEvent('syn_farming:recplants')
AddEventHandler('syn_farming:recplants', function(info)
    for o,p in pairs(info) do 
        if  allfarms[p.k] == nil then 
            allfarms[p.k] = {}
        end
        if allfarms[p.k][p.x] == nil then 
            allfarms[p.k][p.x] = {}
        end
        allfarms[p.k][p.x][p.g] = p.info
    end
end)
...
--rec wagon info
RegisterNetEvent('syn_farming:recinfowagons')
AddEventHandler('syn_farming:recinfowagons', function(x)
    clientwaterwagons = x 
end)


-- on watercan item use
RegisterNetEvent('syn_farming:wateringcan')
AddEventHandler('syn_farming:wateringcan', function()
...
RegisterNetEvent('syn_farming:usebucketinwater')
AddEventHandler('syn_farming:usebucketinwater', function()
    TriggerEvent("vorp_inventory:CloseInv")
    local coords = GetEntityCoords(PlayerPedId())
	...
	if Water == 231313522 or 2005774838 or -1287619521 or -196675805 or -1308233316 or 1755369577 or -2040708515 or -557290573 or -247856387 or 370072007 or -1504425495 or -1369817450 or -1356490953 or -1781130443 or -1300497193 or -1276586360 or -1410384421 or 650214731 or 592454541 or -804804953 or 1245451421 or -218679770 or -1817904483 or -811730579 or -1229593481 or -105598602 then
		if IsEntityInWater(PlayerPedId()) then
            ...
            TriggerServerEvent("syn_farming:watefromriver")  
        else
            TriggerServerEvent("syn_farming:termbucket")  
        end
    else
        TriggerServerEvent("syn_farming:termbucket")  
    end
...
RegisterNetEvent('syn_farming:animationtrigger')
AddEventHandler('syn_farming:animationtrigger', function(type)
    if type == "trim" then 
        trimanim()
...
RegisterNetEvent('syn_farming:createblip')
AddEventHandler('syn_farming:createblip', function(type,coords,idnumber)
...
RegisterNetEvent('syn_farming:usedseed')
AddEventHandler('syn_farming:usedseed', function(type)
    TriggerEvent("vorp_inventory:CloseInv")
    disableinv = true 
    if allowfarming then 
                ...
                if not IsEntityInWater(PlayerPedId()) then  
                    ...
                    if Config.minigame then
                        ...
                        if minigame == 100 then 
                            plantanim()
                            TriggerServerEvent("syn_farming:approvefarming",type,coords)
                        else
        ...
        TriggerServerEvent("syn_farming:cleartofarm")
    end
    disableinv = false 
end)

-- remove plant 
RegisterNetEvent('syn_farming:removeplant')
AddEventHandler('syn_farming:removeplant', function(k, x, g)
    if allfarms[k] and allfarms[k][x] and allfarms[k][x][g] then
        if render[allfarms[k][x][g].render] then
...
RegisterNetEvent('syn_farming:recst')
AddEventHandler('syn_farming:recst', function()
    resp = "done"
end)
-- pump bucket animation
RegisterNetEvent('syn_farming:pumpbucket')
AddEventHandler('syn_farming:pumpbucket', function(waterpumpcoords)
...
RegisterNetEvent('syn_farming:clearonholdbucket')
AddEventHandler('syn_farming:clearonholdbucket', function()
    onholdbucket = false
end)
Citizen.CreateThread(function()
    while true do
    ...
    while true do
        ...
        if 1.5 > pumpdist and not onholdbucket then 
            ...
            if Citizen.InvokeNative(0xC92AC953F0A982AE,pumpwater) then
                TriggerServerEvent("syn_farming:checkbucket",waterpumpcoords)
                onholdbucket = true
                Wait(5000)
            end
        end
                        ...
                        if 1 > dist then 
                            if not Config.onlyowner or charid == k then 
                                ...
                                if Citizen.InvokeNative(0xC92AC953F0A982AE,removeplant) then
                                    removeplantanim()
                                    TriggerServerEvent("syn_farming:interact","remove",h.type,h.render) 
                                    resp = nil 
                                    while resp == nil do 
                                        Wait(1000)
                                    end
                                end
                                if Citizen.InvokeNative(0xC92AC953F0A982AE,update) then
                                    if not updatingnow then 
                                        TriggerServerEvent("syn_farming:requestupdate",h.render,k,x,g) 
                                        updatingnow = true 
                                    end
                                    ...
                                    if Config.drawtext then 
                                        ...
                                        if IsDisabledControlJustReleased(0,Config.keys["G"]) then
                                            if Config.minigame then 
                                                ...
                                                if minigame == 100 then
                                                    harvestanim()
                                                    TriggerServerEvent("syn_farming:interact","harvest",h.type,h.render) 
                                                    resp = nil 
                                                    while resp == nil do 
                                                        Wait(1000)
                                                    end
                                                    ...
                                                    TriggerServerEvent("syn_farming:interact","dmg",h.type,h.render) 
                                                    Wait(1000)
                                                end
                                            else
                                                harvestanim()
                                ...
                                if not h.watered and not h.trem and not holdingcan then 
                                    ...
                                    if Citizen.InvokeNative(0xC92AC953F0A982AE,trim) then
                                        if Config.minigame then 
                                            ...
                                            local minigame = exports["syn_minigame"]:taskBar(diff,7)
                                            if minigame == 100 then
                                                TriggerServerEvent("syn_farming:interact","trim",h.type,h.render) 
                                                resp = nil 
                                                while resp == nil do 
                                                    Wait(1000)
                                                end
                                ...
                                if not h.fert and not h.watered and timetowater ~= h.timer and not holdingcan then 
                                    ...
                                    if Citizen.InvokeNative(0xC92AC953F0A982AE,fert) then
                                        if Config.minigame then
                                            ...
                                            local minigame = exports["syn_minigame"]:taskBar(diff,7)
                                            if minigame == 100 then
                                                TriggerServerEvent("syn_farming:interact","fert",h.type,h.render) 
                                                resp = nil 
                                                while resp == nil do 
                                                    Wait(1000)
                                                end
                                ...
                                if holdingcan and not h.watered and timetowater ==  h.timer then
                                    ...
                                    if Citizen.InvokeNative(0xC92AC953F0A982AE,water2plant) then
                                        if Config.minigame then
                                            ...
                                            if minigame == 100 then
                                                wateranim()
                                                TriggerServerEvent("syn_farming:interact","water",h.type,h.render) 
                                                resp = nil 
                                                while resp == nil do 
                                                    Wait(1000)
                                                end
                    ...
                    if not IsPedInVehicle(PlayerPedId(),closewagon) then 
                        if exists then 
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE,wagonwater) then
                                ...
                                ClearPedTasks(PlayerPedId())
                                bucket()
                                TriggerServerEvent("syn_farming:waterfromwagon",network)
                                resp = nil 
                                while resp == nil do 
                                    Wait(1000)
                                end
                        ...
                        if ( holdingcan or (Water == 231313522 or 2005774838 or -1287619521 or -196675805 or -1308233316 or 1755369577 or -2040708515 or -557290573 or -247856387 or 370072007 or -1504425495 or -1369817450 or -1356490953 or -1781130443 or -1300497193 or -1276586360 or -1410384421 or 650214731 or 592454541 or -804804953 or 1245451421 or -218679770 or -1817904483 or -811730579 or -1229593481 or -105598602) and IsEntityInWater(closewagon)) then
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE,fillwagon) then
                                ...
                                Citizen.Wait(5000)
                                ClearPedTasks(PlayerPedId())
                                TriggerServerEvent("syn_farming:sendwagonfillrequest",k,network,space)
                                resp = nil 
                                while resp == nil do 
                                    Wait(1000)
                                end
