...
local function toNumberSafe(v)
    ...
    v = tostring(v)
    v = v:gsub("%s+", "")   -- remove spaces
    v = v:gsub(",", "")     -- remove commas (e.g. 1,000)
    return tonumber(v)
end
...
function openbank(title)
    ...
    local goldBal = 0
    if bankinfo then
        cashBal = tonumber(bankinfo.money) or 0
        goldBal = tonumber(bankinfo.gold) or 0
    end

    local elements = {
        { label = Config.language.cashbalance .. cashBal, value = 'nothing', desc = Config.language.cashbalance2 },
    }

    if Config.gold then
        table.insert(elements, { label = Config.language.goldbalance .. goldBal, value = 'nothing', desc = Config.language.cashbalance2 })
    end
