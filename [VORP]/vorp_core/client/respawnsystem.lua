...
-- Keeps old scripts working that still listen to:
--   "vorp:PlayerForceRespawn"
...
AddEventHandler("vorp_core:Client:onPlayerRevive", function()
    ...
    -- TriggerEvent("vorp:resurrectPlayer") -- only if you KNOW something uses it
end)
...
local function RespawnTimer()
    ...
    CreateThread(function() -- asynchronous
        while true do
            Wait(1000)
...
function CoreAction.Player.RespawnPlayer(allowCleanItems)
    if allowCleanItems then
        TriggerServerEvent("vorp:PlayerForceRespawn") -- inventory clean items (server-side)
    end

    -- ✅ UPDATED: old client event replaced with new revive event
    -- OLD: TriggerEvent("vorp:PlayerForceRespawn")
...
--========================================================
-- External revive (for syn_medical / other resources)
-- Revives in-place and cleans up death handler state/cam
--========================================================
RegisterNetEvent("vorp_core:Client:ExternalRevive")
AddEventHandler("vorp_core:Client:ExternalRevive", function()
    -- clear death handler state
    setDead = false
    PressKey = true
    Done = false
    TimeToRespawn = Config.RespawnTime

    -- stop death cam safely
    if cam ~= nil then
        EndDeathCam()
    end

    -- revive in-place (no hospital teleport, no ragdoll sequence)
    CoreAction.Player.ResurrectPlayer(nil, nil, true)

    -- fire new revive hook for any listeners (optional but useful)
    TriggerEvent("vorp_core:Client:onPlayerRevive")
end)
